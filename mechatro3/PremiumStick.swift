...
//                calcStick()
    ...
    var body: some View {
        ...
        ZStack{
            ...
            WebView(loardUrl: URL(string: "http://" + SendInputValue.hoststr + ":8000/forward/")!)
    ...
    func startSend(){
        SendInputValue.self.connect()/*sending inter val*/
        timer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { _ in
            let message:String = calcStick()
            SendInputValue.self.send(message.data(using:.utf8)!)
        }
    }
    func stopSend(){
